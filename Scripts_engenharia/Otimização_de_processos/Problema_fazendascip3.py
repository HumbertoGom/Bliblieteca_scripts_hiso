...
model.M = Param(model.IDX_c, model.IDX_f, initialize=Mdict, within=NonNegativeReals)
...
# Função de diagnóstico de violações
def checar_violacoes(m):
    print("\n--- Checando restrições ---")
    for f in m.IDX_f:
        area = sum(value(m.Scf[c,f]) for c in m.IDX_c)
        water = sum(value(m.Scf[c,f]) * value(m.Kc[c]) for c in m.IDX_c)
        print(f"{f} -> área usada {area} (Af = {value(m.Af[f])}), slack = {area - value(m.Af[f])}")
        print(f"{f} -> água usada {water} (Wf = {value(m.Wf[f])}), slack = {water - value(m.Wf[f])}")
    for c in m.IDX_c:
        total = sum(value(m.Scf[c,f]) for f in m.IDX_f)
        print(f"{c} -> área total {total} (Ac = {value(m.Ac[c])}), slack = {total - value(m.Ac[c])}")
    # checar links
    for c in m.IDX_c:
        for f in m.IDX_f:
            scf = value(m.Scf[c,f]); y = value(m.Ycf[c,f]); Mv = value(m.M[c,f])
            if scf > Mv * y + 1e-6:
                print("VIOLAÇÃO LINK:", c, f, "Scf=", scf, "> M*y =", Mv*y)
checar_violacoes(model)